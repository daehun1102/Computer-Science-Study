...
## 네트워크란?
네트워크란 컴퓨터와 다른 하드웨어 구성요소들이 물리적으로나, 논리적으로 연결된 구조를 뜻하며 서로간의 자원이나 정보를 공유하고 교환할 수 있게 해주는 것을 뜻한다.

## 인터넷의 구성요소

1. Devices(장비)<br>인터넷의 끝단에 있는 `호스트`나 `종단 시스템`을 의미한다. 웹 브라우저가 설치된 컴퓨터, 스마트폰, 서버 등 인터넷에 연결되어 정보를 주고받는 모든 장비가 여기에 해당한다.

2. Packet Switches(패킷 스위치)<br>장비들 사이에서 **데이터 패킷을 전달하고 라우팅을 하는 역할**을 한다.
대표적인 패킷 스위치로는 `라우터`와 `스위치`가 있고 이들은 들어온 패킷을 목적지 주소에 따라 다음 경로로 전달하는 역할을 한다.

3. Communication Links(통신 링크)<br>장비와 패킷 스위치, 또는 패킷 스위치와 패킷 스위치를 물리적으로 연결하는 통신 선로다. `광케이블`, `위성 통신`, `무선 주파수` 등 다양한 형태가 있으며 이 링크를 통해 데이터가 패킷의 형태로 전송된다.

4. Networks(네트워크)<br>위에서 언급한 모든 구성 요소들이 연결되어 하나의 거대한 통신망을 이룬 것을 통칭한다. 이 개념은 단순히 물리적 연결을 넘어, IP 주소 체계나 프로토콜(통신 규약) 같은 논리적인 구조까지 포함하는 포괄적인 개념이다.

<br>

> **인터넷은 데이터를 주고받는 장비들이 통신링크를 통해 패킷 스위치의 도움을 받아 서로 연결되어 있는 거대한 네트워크이다.**

## 프로토콜의 개념

프로토콜은 네트워크에서 데이터를 주고받기 위한 일련의 규칙과 절차를 뜻한다.

사람들이 소통을 할 때 언어를 사용하는 것처럼, 컴퓨터와 네트워크 장비들도 원활하게 통신을 하기 위해서 프로토콜이라는 규약을 따른다.

## Internet Protocol Stack

인터넷 프로토콜 스택은 네트워크 통신을 기능별로 계층화한 모델이며 우리가 말하는 TCP/IP 모델이 이 프로토콜 스택을 뜻한다.

> ### TCP/IP는 4계층 아닌가요?

Physical Layer와 Link Layer를 하나로 묶어서 네트워크 접근 계층으로 본다면 4계층이고, 그렇지 않다면 5계층이라고 말할 수도 있다.

## Application Layer(애플리케이션 계층)
사용자가 네트워크에 직접 접근하는 최상위 계층으로, 웹 브라우저나 이메일 프로그램, 파일 전송 프로그램 등이 이 계층에 해당하며 FTP, SMTP, HTTP와 같은 특정 서비스 프로토콜을 사용한다.

## Transport Layer(트랜스포트 계층)
애플리케이션 계층 간에 데이터를 안정적으로 전송하는 역할을 한다.
...
패킷을 목적지 네트워크까지 전송하는 역할을 한다.

`Host-to-Host` 통신을 담당한다.

**IP(Internet Protocol)** 가 이 계층의 핵심 프로토콜이며, 패킷의 **경로(Routing)** 를 결정하고, 논리적인 주소인 IP 주소를 사용한다.
